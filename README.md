...
Имеется кусок конвейера (PB_1 – пуск/стоп (DI), 1 DO команда на привод без обратной связи), 
два датчика SO1 и SO2 (2 DI) и отбраковщик (1 DO пушер без обратной связи).
...
По датчику SO2 должна произойти проверка поля «Code» через 500 импульсов энкодера относительно срабатывания датчика SO2, 
на ее основании продукт должен отбраковаться или проехать дальше. Удаляется элемент по принципу FIFO.
При переполнении очереди выдается ошибка (пока очередь является переполненной), конвейер останавливается. 
Предусмотреть бит в программе для очистки всей очереди.
